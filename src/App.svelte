<script>
  ...
  import Toggler from './component/Toggler.svelte';
  ...
  const Routes = RouteManager.formValues;

  export let emptyRoute = {
    title: '',
    url: '',
    tags: [],
  };

  export let initalRoutes = [
    {
      title: 'Brave Search',
  ...
  RouteManager.init(initalRoutes);

  addRoute(emptyRoute);

  function save(payload) {
    console.log('RouteManager.formValues: ', RouteManager.formValues);

    return RouteManager;
  }
  
  function addRoute(payload) {
    RouteManager.add(payload || emptyRoute);
  }
...
<main>
  <img src={logo} alt="Svelte Logo" class="mx-auto h-12 w-auto" />
